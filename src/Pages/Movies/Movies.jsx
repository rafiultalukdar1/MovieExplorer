        ...
        <>
                ...
                <div className="container">
                    ...
                    <div>
                        <h1 className='text-3xl sm:text-4xl md:text-[50px] font-bold text-white'>
                            Explore{' '}
                            <span className='text-[#2DD4BF]'>Movies</span>
                        </h1>
                    ...
                    <div className='flex justify-between flex-col lg:flex-row items-center mt-8 gap-5'>
                        ...
                        <div className='order-1 lg:order-2 shrink-0 w-full max-w-97.5'>
                            <div className='relative w-full'>
                                <span className='absolute left-4 top-1/2 -translate-y-1/2 text-[#2DD4BF] text-xl'>
                                    <GoSearch />
                                </span>
                                <input type='text' value={search} onChange={handleSearch} placeholder='Search for a movie...' className='w-full bg-[#141414] border border-[#ffffff26] rounded-lg py-2.5 pl-12 pr-5 text-white outline-none focus:border-[#2DD4BF]' />
                            </div>
                        </div>
                        ...
                        <div className='flex flex-wrap justify-center gap-2 order-2 lg:order-1'>
                            {genres.map((genre) => (
                                <button key={genre} onClick={() => handleGenreFilter(genre) } className={`py-2 px-3.75 sm:px-5 rounded-sm border text-sm font-medium cursor-pointer transition ${ activeTab === genre ? 'bg-[#2DD4BF] border-[#2DD4BF] text-[#0C0C0C]' : 'bg-transparent border-[#ffffff26] text-white hover:border-[#2DD4BF] hover:text-[#2DD4BF]' }`}>{genre}</button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className='mt-12'>
                        {loading ? (
                            <div className='flex justify-center items-center py-20'>
                                <div className='w-10 h-10 border-4 border-[#ffffff26] border-t-[#2DD4BF] rounded-full animate-spin'></div>
                            </div>
                        ) : movies.length === 0 ? (
                            ...
                            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                                {movies.map((movie) => (
                                    <div key={movie.id} className='group bg-[#141414] border border-[#ffffff1a] rounded-lg overflow-hidden hover:border-[#2DD4BF] transition-all duration-300'>

                                        {/* Poster */}
                                        <div className='h-65 overflow-hidden bg-[#0C0C0C]'>
                                            {movie.image?.medium ? (
                                        ...
                                        <div className='p-4'>
                                            ...
                                            <div className='flex items-center justify-between mt-3'>
                                                <span className='flex items-center gap-1 text-[#2DD4BF] text-[18px]'>
                                                    <FaStar />{' '}
                                            ...
                                            <button onClick={() => setSelectedMovie(movie) } className='w-full mt-4 py-2 border border-[#2DD4BF] text-[#2DD4BF] hover:bg-[#2DD4BF] hover:text-[#0C0C0C] rounded-sm font-medium transition cursor-pointer'>See Details</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            {selectedMovie && (
                <div onClick={() => setSelectedMovie(null)} className='fixed inset-0 z-99 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4'>
                    <div onClick={(e) => e.stopPropagation()} className='relative w-full max-w-222 max-h-[90vh] overflow-y-auto bg-[#141414] border border-[#ffffff26] rounded-lg'>
                        {/* Close Button */}
                        <button onClick={() => setSelectedMovie(null)} className='absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center bg-[#2DD4BF] text-black rounded-full cursor-pointer transition text-xl'>
                            <IoClose />
                        </button>
                        ...
                        <div className="p-5 sm:p-7">
                            ...
                            <div className='flex flex-wrap items-center gap-5 mt-2 md:mt-4 text-sm'>
                                <span className='flex items-center gap-1 text-[#2DD4BF] text-[16px]'>
                                    <FaStar />{' '}
                            ...
                            {selectedMovie.genres?.length > 0 && (
                                <div className='flex flex-wrap gap-2 mt-3.75'>
                                    {selectedMovie.genres.map(
                                        (genre) => (
                                            <span key={genre} className='px-3 py-1 text-xs text-[#2DD4BF] bg-[#2dd4be1a] border border-[#2dd4bf40] rounded-full' >
                                                {genre}
                                            </span>
                                        )
                                    )}
                            ...
                            <div className='text-end'>
                                <button onClick={() => setSelectedMovie(null)} className='mt-7.5 py-2 px-7 bg-[#2DD4BF] text-[#0C0C0C] font-semibold rounded-sm cursor-pointer' >Close</button>
                            </div>
                        </div>
                    </div>
                </div>
